...
export default function NetCDFVisualizer() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("/api/parse-nc", {
        method: "POST",
        body: formData,
      })

      if (!response.ok) {
        throw new Error("Failed to process NetCDF file")
      }
  ...
  return (
    <div className="container mx-auto p-6 max-w-7xl">
      <div className="mb-8 bg-gradient-to-r from-indigo-600 to-purple-600 p-6 rounded-xl shadow-lg backdrop-blur-sm bg-opacity-20">
        <h1 className="text-3xl font-bold mb-2 text-white">NetCDF Visualizer</h1>
        <p className="text-white/90">Upload and visualize NetCDF files with interactive charts and plots</p>
      </div>
      ...
      <Card className="mb-6 backdrop-blur-sm bg-white/10 border border-white/20">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <Label htmlFor="file-upload" className="text-gray-800/80">Select NetCDF File (.nc)</Label>
              <Input
                id="file-upload"
                type="file"
                ref={fileInputRef}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-1 space-y-6">
              {/* Variables Tab List */}
              <Card className="backdrop-blur-sm bg-white/10 border border-white/20 h-full">
                <CardHeader>
                  <CardTitle className="text-white bg-gradient-to-r from-indigo-500 to-purple-500 p-2 rounded-lg">Variables</CardTitle>
                </CardHeader>
                <CardContent>
                  <Tabs
                    ...
                    <TabsList className="grid gap-2 w-full bg-transparent justify-start">
                      {Object.keys(ncData.variables).map((varName) => (
                        ...
                        >
                          ...
                          {/* <div className="text-xs opacity-70">{getVariableInfo(varName)}</div> */}
                        </TabsTrigger>
                      ))}
                    </TabsList>
                  </Tabs>
            ...
            <div className="lg:col-span-3">
              <Tabs defaultValue="plot" className="w-full" onValueChange={(value) => setActiveTab(value)}>
                ...
                {activeTab === "plot" && (<TabsContent value="plot">
                  <Card className="backdrop-blur-sm bg-white/10 border border-white/20">
                    <CardHeader>
                      <CardTitle className="text-black">
                        {selectedVariable ? `${selectedVariable} - ${plotType}` : "Select a variable to visualize"}
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                      {selectedVariable ? (
                        ...
                        <div className="flex items-center justify-center h-96 text-black/70">
                          Select a variable and plot type to begin visualization
                        </div>
                      )}
                    </CardContent>
                  </Card>
                ...
                <TabsContent value="metadata">
                  <Card className="backdrop-blur-sm bg-white/10 border-2 border-gradient-to-r from-indigo-500 to-purple-500/20">
                    <CardHeader>
                      <CardTitle className="text-black">File Metadata</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="bg-white/5 p-3 rounded-md max-h-64 overflow-y-auto border border-white/10">
                          <pre className="text-sm text-black/90">{JSON.stringify(ncData.metadata.globalAttributes, null, 2)}</pre>
                        </div>
                      </div>
