...
const parsePilotData = (data: string) => {
  ...
  const stationInfo = {
    ...
    coordinates: {
      lat: Number.parseFloat(extractValue(lines, "GroundLat") || extractValue(lines, "Lat") || "23.779702"),
      lng: Number.parseFloat(extractValue(lines, "GroundLng") || extractValue(lines, "Lng") || "90.379230"),
    },
    elevation: Number.parseFloat(extractValue(lines, "GroundAlt") || extractValue(lines, "Elevation") || "11"),
    date: extractValue(lines, "Date") || new Date().toISOString().split("T")[0],
    time: extractValue(lines, "Time") || "12:00",
    groundPressure: Number.parseFloat(extractValue(lines, "GroundP") || "1010.6"),
    groundTemperature: Number.parseFloat(extractValue(lines, "GroundT") || "20.0"),
  ...
  const groundConditions = {
    pressure: stationInfo.groundPressure || atmosphericData[0]?.pressure || 1010.6,
    temperature: stationInfo.groundTemperature || atmosphericData[0]?.temperature || 20.0,
    maxAltitude: stationInfo.maxAltitude || Math.max(...atmosphericData.map((d) => d.altitude)) || 12197,
  }
...
const parseDemsData = (data: string) => {
  const lines = data.split("\n").filter((line) => line.trim())
  const demsMessages = lines.filter((line) => line.includes("DEMS") || line.includes("TTAA"))
...
const parseCsvData = (data: string) => {
    ...
    .map((line) => {
      ...
      headers.forEach((header, index) => {
        ...
        if (!isNaN(value)) {
          ...
          if (header.includes("alt") || header.includes("height")) entry.altitude = value
          else if (header.includes("press") || header.includes("hpa")) entry.pressure = value
          else if (header.includes("temp") || header.includes("celsius")) entry.temperature = value
          else if (header.includes("humid") || header.includes("rh")) entry.humidity = value
          else if (header.includes("wind") && header.includes("speed")) entry.windSpeed = value
          else if (header.includes("wind") && header.includes("dir")) entry.windDir = value
  ...
  return {
    ...
    atmosphericProfile: atmosphericData,
    dataQuality: calculateDataQuality(atmosphericData),
    messageTypes: [{ type: "CSV", count: atmosphericData.length, percentage: 100 }],
    groundConditions,
  }
}
...
const extractValue = (lines: string[], key: string): string | null => {
  for (const line of lines) {
    if (line.includes(key)) {
      ...
      const match = line.match(new RegExp(`${key}[:\\s\$$][^\$$]*\\)?[:\\s]+([\\d\\.\\-]+)`, "i"))
      if (match) return match[1]

      // Try splitting by colon or space
      const parts = line.split(/[:\s]+/)
      const keyIndex = parts.findIndex((part) => part.toLowerCase().includes(key.toLowerCase()))
...
const extractAtmosphericProfile = (lines: string[]) => {
  const dataLines = lines.filter((line) => {
    ...
    const nums = line
      .split(/\s+/)
      .filter((part) => !isNaN(Number.parseFloat(part)) && isFinite(Number.parseFloat(part)))
...
const dewpointToHumidity = (temperature: number, dewpointDep: number): number => {
...
const autoDetectAndParse = (data: string) => {
  if (data.includes("PILOT") || data.includes("DHAKA") || data.includes("Station")) {
    ...
    throw new Error("Unable to detect data format. Please specify the format manually.")
  }
}
...
const calculateDataQuality = (data: any[]) => {
  ...
  const optionalQuality = totalFields > 0 ? (validFields / totalFields) * 100 : 100
...
const analyzeMessageTypes = (lines: string[]) => {
  const types = new Map()
  lines.forEach((line) => {
    if (line.includes("PILOT")) types.set("PILOT", (types.get("PILOT") || 0) + 1)
...
const calculateStatistics = (data: any[], field: string) => {
  const values = data.map((d) => d[field]).filter((v) => v !== null && v !== undefined && !isNaN(v) && isFinite(v))
...
12197 150.0 -65.0 15 48.0 355`
  ...
  const statistics = parsedData
    ? {
        temperature: calculateStatistics(parsedData.atmosphericProfile, "temperature"),
        pressure: calculateStatistics(parsedData.atmosphericProfile, "pressure"),
        humidity: calculateStatistics(parsedData.atmosphericProfile, "humidity"),
        windSpeed: calculateStatistics(parsedData.atmosphericProfile, "windSpeed"),
      }
    : null
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-4">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-gray-600">International Standard Meteorological Analysis System</p>
          ...
          <TabsContent value="input" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Upload meteorological data files (TXT, CSV, or other formats)</CardDescription>
                </CardHeader>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Copy and paste your meteorological data directly</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={handleParseData} disabled={isLoading} className="flex-1">
                      {isLoading ? (
                        <>
            ...
            {parsedData && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Preview of your atmospheric sounding data with corrected calculations
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="text-center p-4 bg-blue-50 rounded-lg">
                      <p className="text-2xl font-bold text-blue-600">{parsedData.atmosphericProfile.length}</p>
                      <p className="text-sm text-gray-600">Measurement Levels</p>
                    </div>
                    <div className="text-center p-4 bg-green-50 rounded-lg">
                      <p className="text-2xl font-bold text-green-600">{parsedData.dataQuality}%</p>
                      <p className="text-sm text-gray-600">Data Quality</p>
                    </div>
                    <div className="text-center p-4 bg-purple-50 rounded-lg">
                      <p className="text-2xl font-bold text-purple-600">{parsedData.stationInfo.name}</p>
                      <p className="text-sm text-gray-600">Station</p>
                    </div>
                    <div className="text-center p-4 bg-orange-50 rounded-lg">
                      <p className="text-2xl font-bold text-orange-600">{parsedData.messageTypes.length}</p>
                      <p className="text-sm text-gray-600">Message Types</p>
                    </div>
                  </div>

                  <div className="mt-4 text-center">
                    <Button onClick={() => setActiveTab("profiles")}>View Analysis Results</Button>
                  </div>
                </CardContent>
              </Card>
            )}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>WMO compliant meteorological data formats</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className="text-sm text-gray-600 mb-2">
                      WMO standard for upper-air wind data from pilot balloon observations
                    </p>
                  ...
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className="text-sm text-gray-600 mb-2">
                      Standard format with altitude, pressure, temperature, humidity, wind data
                    </p>
            ...
            <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                          ...
                          <p className="text-2xl font-bold text-blue-600">
                            {formatValue(
                              parsedData.stationInfo.maxAltitude ||
                                Math.max(...parsedData.atmosphericProfile.map((d) => d.altitude)),
                              "m",
                              0,
                            )}
                          </p>
                          <p className="text-xs text-gray-500">
                            {(parsedData.stationInfo.maxAltitude / 1000).toFixed(1)} km AGL
                          </p>
                        </div>
                  ...
                  <Card>
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-gray-600">Ground Temperature</p>
                          <p className="text-2xl font-bold text-red-600">
                            {formatValue(
                              parsedData.stationInfo.groundTemperature || parsedData.atmosphericProfile[0]?.temperature,
                              "°C",
                            )}
                          </p>
                          <p className="text-xs text-gray-500">
                  ...
                  <Card>
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-gray-600">Ground Pressure</p>
                          <p className="text-2xl font-bold text-green-600">
                            {formatValue(
                              parsedData.stationInfo.groundPressure || parsedData.atmosphericProfile[0]?.pressure,
                              "hPa",
                            )}
                          </p>
                          <p className="text-xs text-gray-500">
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-2xl font-bold text-purple-600">{parsedData.dataQuality}%</p>
                          <p className="text-xs text-gray-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2 text-sm">
                          <p>
                            <span className="font-medium">Station:</span> {parsedData.stationInfo.name}
                          </p>
                          <p>
                            <span className="font-medium">Country:</span> {parsedData.stationInfo.country}
                          </p>
                          <p>
                            <span className="font-medium">WMO ID:</span> {parsedData.stationInfo.wmoId}
                          </p>
                          <p>
                            <span className="font-medium">Elevation:</span>{" "}
                            {formatValue(parsedData.stationInfo.elevation, "m AMSL", 0)}
                          </p>
                        </div>
                      </div>
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2 text-sm">
                          <p>
                            ...
                            {parsedData.stationInfo.coordinates.lat.toFixed(6)}°N
                          </p>
                          <p>
                            <span className="font-medium">Longitude:</span>{" "}
                            {parsedData.stationInfo.coordinates.lng.toFixed(6)}°E
                          </p>
                          <p>
                            <span className="font-medium">Format:</span> WGS84 Decimal Degrees
                          </p>
                        </div>
                      </div>
                      <div className="space-y-3">
                        <h4 className="font-medium text-gray-700">Observation</h4>
                        <div className="space-y-2 text-sm">
                          <p>
                            <span className="font-medium">Date:</span> {parsedData.stationInfo.date} (ISO 8601)
                          </p>
                          <p>
                            <span className="font-medium">Time:</span> {parsedData.stationInfo.time} UTC
                          </p>
                          <p>
                            <span className="font-medium">Type:</span> Upper-air Sounding
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Temperature variation with altitude (International Standard Atmosphere)
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={400}>
                        <LineChart data={parsedData.atmosphericProfile}>
                          ...
                          <YAxis
                            dataKey="altitude"
                            label={{ value: "Altitude (m AMSL)", angle: -90, position: "insideLeft" }}
                          />
                          <Tooltip
                            formatter={(value, name) => [
                              name === "temperature" ? `${value}°C (${((value * 9) / 5 + 32).toFixed(1)}°F)` : value,
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Atmospheric pressure with altitude (hPa/mb)</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={400}>
                        <AreaChart data={parsedData.atmosphericProfile}>
                          ...
                          <XAxis
                            dataKey="altitude"
                            label={{ value: "Altitude (m AMSL)", position: "insideBottom", offset: -5 }}
                          />
                          <YAxis label={{ value: "Pressure (hPa)", angle: -90, position: "insideLeft" }} />
                          <Tooltip
                            formatter={(value, name) => [
                              name === "pressure" ? `${value} hPa (${(value * 0.02953).toFixed(2)} inHg)` : value,
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Relative humidity variation (%RH)</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={400}>
                        <BarChart data={parsedData.atmosphericProfile.filter((d) => d.humidity !== null).slice(0, 12)}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis
                            dataKey="altitude"
                            label={{ value: "Altitude (m)", position: "insideBottom", offset: -5 }}
                          />
                          <YAxis
                            label={{ value: "Relative Humidity (%)", angle: -90, position: "insideLeft" }}
                            domain={[0, 100]}
                          />
                          <Tooltip formatter={(value) => [`${value}%`, "Relative Humidity"]} />
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Temperature and pressure combined (International Units)</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={400}>
                        <LineChart data={parsedData.atmosphericProfile}>
                          ...
                          <Tooltip
                            formatter={(value, name) => [
                              name === "temperature" ? `${value}°C` : `${value} hPa`,
                              name === "temperature" ? "Temperature" : "Pressure",
                            ]}
                          />
                          <Legend />
                          <Line
                ...
                {statistics && (
                  <>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <Card>
                        <CardHeader>
                          <CardTitle>Statistical Summary (International Units)</CardTitle>
                          <CardDescription>
                            Key statistics for atmospheric parameters with proper calculations
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                              ...
                              <div key={param} className="space-y-2">
                                ...
                                <div className="grid grid-cols-4 gap-2 text-sm">
                                  <div className="text-center p-2 bg-blue-50 rounded">
                                    <p className="font-medium text-blue-700">{stats.min}</p>
                                    <p className="text-xs text-gray-600">Minimum</p>
                                  </div>
                                  <div className="text-center p-2 bg-red-50 rounded">
                                    <p className="font-medium text-red-700">{stats.max}</p>
                                    <p className="text-xs text-gray-600">Maximum</p>
                                  </div>
                                  <div className="text-center p-2 bg-green-50 rounded">
                                    <p className="font-medium text-green-700">{stats.avg}</p>
                                    <p className="text-xs text-gray-600">Mean</p>
                                  </div>
                                  <div className="text-center p-2 bg-purple-50 rounded">
                                    <p className="font-medium text-purple-700">{stats.std}</p>
                      ...
                      <Card>
                        <CardHeader>
                          ...
                          <CardDescription>Relationship following International Standard Atmosphere</CardDescription>
                        </CardHeader>
                        <CardContent>
                          <ResponsiveContainer width="100%" height={400}>
                            <ScatterChart
                              data={parsedData.atmosphericProfile.filter(
                                (d) => d.temperature !== null && d.pressure !== null,
                              )}
                            >
                              <CartesianGrid strokeDasharray="3 3" />
                              <XAxis
                                dataKey="pressure"
                                label={{ value: "Pressure (hPa)", position: "insideBottom", offset: -5 }}
                              />
                              <YAxis
                                dataKey="temperature"
                                label={{ value: "Temperature (°C)", angle: -90, position: "insideLeft" }}
                    ...
                    <Card>
                      <CardHeader>
                        <CardTitle>Atmospheric Layer Analysis (ICAO Standard Atmosphere)</CardTitle>
                        <CardDescription>
                          Identification of atmospheric layers based on temperature gradient and international standards
                        </CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div className="p-4 bg-blue-50 rounded-lg">
                            <h4 className="font-medium text-blue-800">Troposphere</h4>
                            <p className="text-sm text-blue-600 mt-1">
                              Surface - {Math.min(11000, parsedData.stationInfo.maxAltitude).toLocaleString()} m
                            </p>
                            <p className="text-xs text-gray-600 mt-2">Temperature decreases with altitude</p>
                            <div className="mt-2 space-y-1">
                              <p className="text-xs">Standard Lapse Rate: 6.5°C/km</p>
                              <p className="text-xs">
                                Observed Range: {statistics.temperature.min}°C to {statistics.temperature.max}°C
                              </p>
                            </div>
                          </div>
                          ...
                          {parsedData.stationInfo.maxAltitude > 11000 && (
                            <div className="p-4 bg-purple-50 rounded-lg">
                              <h4 className="font-medium text-purple-800">Tropopause</h4>
                              <p className="text-sm text-purple-600 mt-1">~11,000 - 20,000 m</p>
                              <p className="text-xs text-gray-600 mt-2">Temperature stabilization zone</p>
                              <div className="mt-2 space-y-1">
                                <p className="text-xs">Standard Temp: -56.5°C</p>
                          ...
                          <div className="p-4 bg-green-50 rounded-lg">
                            <h4 className="font-medium text-green-800">Data Coverage</h4>
                            <p className="text-sm text-green-600 mt-1">
                              {(parsedData.stationInfo.maxAltitude / 1000).toFixed(1)} km altitude
                            </p>
                            <p className="text-xs text-gray-600 mt-2">Quality assessment</p>
                            <div className="mt-2 space-y-1">
                              <p className="text-xs">Data Quality: {parsedData.dataQuality}%</p>
                              <p className="text-xs">Measurements: {parsedData.atmosphericProfile.length} levels</p>
                            </div>
                          </div>
                        </div>
                      </CardContent>
              ...
              <TabsContent value="dems" className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle>Message Type Distribution (WMO Standards)</CardTitle>
                      <CardDescription>Distribution of meteorological message types in your data</CardDescription>
                    </CardHeader>
                    <CardContent>
                  ...
                  <Card>
                    <CardHeader>
                      <CardTitle>Data Quality Assessment (International Standards)</CardTitle>
                      <CardDescription>Quality metrics following WMO guidelines</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span>Overall Data Quality</span>
                          <span className="font-medium">{parsedData.dataQuality}%</span>
                        </div>
                        <Progress value={parsedData.dataQuality} className="h-2" />
                      </div>

                      <div className="grid grid-cols-2 gap-4 pt-4">
                        <div className="text-center p-3 bg-green-50 rounded-lg">
                          <p className="text-2xl font-bold text-green-700">{parsedData.atmosphericProfile.length}</p>
                          <p className="text-sm text-gray-600">Pressure Levels</p>
                        </div>
                        <div className="text-center p-3 bg-blue-50 rounded-lg">
                          <p className="text-2xl font-bold text-blue-700">{parsedData.messageTypes.length}</p>
                          <p className="text-sm text-gray-600">Message Types</p>
                        </div>
                      </div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-center">
                        <div className="text-4xl font-bold text-green-600 mb-2">{parsedData.dataQuality}%</div>
                        ...
                        <Progress value={parsedData.dataQuality} className="mt-4" />
                      </div>
                    </CardContent>
                  </Card>
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {Math.min(...parsedData.atmosphericProfile.map((d) => d.altitude))} -{" "}
                            {Math.max(...parsedData.atmosphericProfile.map((d) => d.altitude))} m AMSL
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm">Pressure Range</span>
                          <span className="font-medium">
                            {Math.min(...parsedData.atmosphericProfile.map((d) => d.pressure)).toFixed(1)} -{" "}
                            {Math.max(...parsedData.atmosphericProfile.map((d) => d.pressure)).toFixed(1)} hPa
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm">Temperature Range</span>
                          <span className="font-medium">
                            {Math.min(...parsedData.atmosphericProfile.map((d) => d.temperature)).toFixed(1)} -{" "}
                            {Math.max(...parsedData.atmosphericProfile.map((d) => d.temperature)).toFixed(1)} °C
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm">Measurement Points</span>
                          <span className="font-medium">{parsedData.atmosphericProfile.length}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
              ...
              <TabsContent value="wind" className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Wind speed variation with altitude (m/s, International Standard)
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={400}>
                        <LineChart data={parsedData.atmosphericProfile.filter((d) => d.windSpeed !== null)}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis
                            dataKey="altitude"
                            label={{ value: "Altitude (m AMSL)", position: "insideBottom", offset: -5 }}
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>Wind direction changes with altitude (degrees from North)</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <ResponsiveContainer width="100%" height={400}>
                        <LineChart data={parsedData.atmosphericProfile.filter((d) => d.windDir !== null)}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis
                            dataKey="altitude"
                            label={{ value: "Altitude (m AMSL)", position: "insideBottom", offset: -5 }}
                          />
                          <YAxis
                            label={{ value: "Wind Direction (°)", angle: -90, position: "insideLeft" }}
                            domain={[0, 360]}
                          />
                          <Tooltip
                            formatter={(value) => [`${value}° (${getWindDirectionName(value)})`, "Wind Direction"]}
                          />
                          <Line
                            type="monotone"
                            dataKey="windDir"
                ...
                {statistics && (
                  <Card>
                    <CardHeader>
                      <CardTitle>Wind Analysis Summary (International Standards)</CardTitle>
                      <CardDescription>Key wind characteristics from your atmospheric sounding data</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="text-center p-4 bg-cyan-50 rounded-lg">
                          <Wind className="h-8 w-8 text-cyan-600 mx-auto mb-2" />
                          <p className="text-2xl font-bold text-cyan-700">{statistics.windSpeed.max} m/s</p>
                          <p className="text-sm text-gray-600">Max Wind Speed</p>
                          <p className="text-xs text-gray-500">{(statistics.windSpeed.max * 1.944).toFixed(1)} kt</p>
                        </div>
                        <div className="text-center p-4 bg-blue-50 rounded-lg">
                          <Activity className="h-8 w-8 text-blue-600 mx-auto mb-2" />
                          <p className="text-2xl font-bold text-blue-700">{statistics.windSpeed.avg} m/s</p>
                          <p className="text-sm text-gray-600">Average Speed</p>
                          <p className="text-xs text-gray-500">{(statistics.windSpeed.avg * 3.6).toFixed(1)} km/h</p>
                        </div>
                        <div className="text-center p-4 bg-purple-50 rounded-lg">
                          <TrendingUp className="h-8 w-8 text-purple-600 mx-auto mb-2" />
                          <p className="text-2xl font-bold text-purple-700">{statistics.windSpeed.min} m/s</p>
                          <p className="text-sm text-gray-600">Min Speed</p>
                          <p className="text-xs text-gray-500">{(statistics.windSpeed.min * 2.237).toFixed(1)} mph</p>
                        </div>
                        <div className="text-center p-4 bg-green-50 rounded-lg">
                          <CheckCircle className="h-8 w-8 text-green-600 mx-auto mb-2" />
                          <p className="text-2xl font-bold text-green-700">{statistics.windSpeed.std}</p>
                          <p className="text-sm text-gray-600">Variability (σ)</p>
        ...
        {parsedData && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Download your corrected analysis in various international standard formats
              </CardDescription>
            </CardHeader>
            <CardContent>
